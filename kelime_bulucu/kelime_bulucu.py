...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        self.title('Kelime Bulucu')
        self.iconbitmap('./kelime_bulucu/icons/sozluk.ico')
        self.iconbitmap()
        self.size(350,300)
        self.main_screen()
    ...
    def main_screen(self):
        ...
        self.str_image = tk.PhotoImage(file='./kelime_bulucu/images/start.png')
        ...
        self.main_title = tk.Label(self.title_frame, text="Kelime Bulucu Oyununa Hoşgeldiniz!", bg="indigo", fg="Lavender", anchor=tk.CENTER, font=("Helvetica", 12))
        self.main_title.pack(expand=True,fill=tk.BOTH)

        self.msg_frame = tk.Frame(self.main_frame,bg="Lavender")
        self.main_msg = tk.Label(self.msg_frame, text="Oyunu kazanabilmek için\n\nkelimeyi doğru tahmin etmelisiniz.\n\nHadi tahmin etmeye başlayalım.",bg="darkblue",fg="Lavender",font=("Helvetica", 12))
        ...
        self.emp.pack()
        self.btn_msg = tk.Label(self.btn_frame, text="Başlamak için lütfen butona basınız.",bg="darkgreen",fg="Lavender",font=("Helvetica", 12))
    ...
    def rand_kelime(self):
        ...
        self.word, self.mean = random.choice(list(sozluk.my_dict.items()))
    ...
    def top_frame_func(self):
        ...
        self.guess_msg = tk.Label(self.toplefttop_frame, text="Lütfen tahmin edeceğiniz\nkelimeyi giriniz. ",bg="SteelBlue",fg="Lavender",font=("Helvetica", 15))
        ...
        self.guess_msg = tk.Label(self.topleftbottom_frame, text=str(self.guess_right)+" tahmin hakkınız kaldı!",bg="SteelBlue",fg="Lavender",font=("Helvetica", 15))
        self.guess_msg.pack(fill=tk.BOTH,expand=True)
    ...
    def main(self):
        ...
        if len(self.guess_text.get("1.0",tk.END).strip()) == 0:
            showwarning(
                title='Uyarı',
                message='Lütfen herhangi bir kelime tahmini yapınız!')
        ...
        # when the guess word does not have five letters
            ...
            showwarning(
                title='Uyarı',
                message="Lütfen beş haneli bir kelime tahmini yapınız!")
            self.guess_text.delete("1.0",tk.END)
            return False
        
        # when the guess word's first letter is not ğ or Ğ
        elif self.guess_arr[0] == "Ğ" or self.guess_arr[0] == "ğ":
            showwarning(
                title='Uyarı',
                message="Tahmin ettiğiniz kelimenin baş harfi 'Ğ' ile başlayamaz!")
            self.guess_text.delete("1.0",tk.END)
            return False
        ...
        # when the word does not have big letter 
            ...
            showwarning(
                title='Uyarı',
                message='Lütfen büyük harfler kullanınız!')
            self.guess_text.delete("1.0",tk.END)
            return False

        # when the word does not make sense 
        elif self.guess_text.get("1.0",tk.END).strip() not in list(sozluk.my_dict.keys()):
            showwarning(
                title='Uyarı',
                message='Lütfen sözlükte bulunan bir kelime tahmini yapınız!')
            self.guess_text.delete("1.0",tk.END)
            return False
            ...
            if(self.guess_text.get("1.0",tk.END) in self.guess_textbx.get("1.0",tk.END)):
                showinfo(
                    title='Uyarı',
                    message='Bu kelimeyi daha önce tahmin ettiniz!')
                self.guess_text.delete("1.0",tk.END)
                return False
            else:
    ...
    def paint(self):
        ...
        if self.word[0] == self.guess_arr[0] and self.word[1] == self.guess_arr[1] and self.word[2] == self.guess_arr[2] and self.word[3] == self.guess_arr[3] and self.word[4] == self.guess_arr[4]:
            showinfo(
                title='Tebrikler',
                message='"Kelime Bulma" oyununu kazandınız.')
            
            answer = askyesno(title='Teklif',
                message='Tekrar oynamak ister misiniz?')
            if answer:
                # want to play again
                self.guess_right = 6
                self.line = 0
        ...
        # when there is no guess right
            ...
            showinfo(
                title='Oyun Bitti!',
                message="Bulunması gereken kelime: \""+self.word+"\"\n\nMaalesef hiçbir tahmin hakkınız kalmadı.")
            ...
            if self.guess_right == 3:
                self.guess_msg.configure(text=str(self.guess_right)+" hakkınız kaldı!")

                showinfo(
                    title='Tahmin Hakkı!',
                    message="Üç tahmin hakkınız kaldı!")

                answer = askyesno(title='Teklif',
                    message='İpucu ister misiniz?')
                if answer:
                    ...
                    self.guess_msg.configure(text=str(self.guess_right)+" tahmin hakkınız kaldı!")
